...
pub(super) fn preflight(cwd: &Path, requested: Option<&str>, tool: &str) -> Result<()> {
    if let Some(requested) = requested {
        if requested.trim().is_empty() || requested.len() > 4096 {
            return Err(Error::tool(tool, "output_path must be nonempty and at most 4096 bytes"));
        }
        ...
        match std::fs::symlink_metadata(&path) {
            Ok(_) => return Err(Error::tool(tool, "output_path already exists; choose a new file")),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
            Err(error) => return Err(Error::tool(tool, format!("cannot inspect output_path: {error}"))),
        }
    }
    Ok(())
}
...
pub(super) fn publish(
    ...
    if bytes.is_empty() {
        return Err(Error::tool(tool, "refusing to publish an empty media artifact"));
    }
    preflight(cwd, requested, tool)?;
    let path = match requested {
        Some(requested) => cwd.join(requested),
        None => cwd.join(format!("{prefix}_{}.{extension}", uuid::Uuid::new_v4().simple())),
    };
    ...
    if !matches {
        return Err(Error::tool(tool, format!(
    ...
    let mut staged = tempfile::Builder::new().prefix(".pi-media-").tempfile_in(parent)
    ...
    staged.persist_noclobber(&path)
        .map_err(|error| Error::tool(tool, format!("cannot publish media artifact without overwriting: {}", error.error)))?;
    Ok(path)
}
...
mod tests {
    ...
    fn publication_never_overwrites_an_existing_file() {
        ...
        let path = publish(dir.path(), Some("result.png"), "images/result", "png", b"original", None, "generate_image").unwrap();
        assert!(publish(dir.path(), Some("result.png"), "images/result", "png", b"replacement", None, "generate_image").is_err());
    ...
    fn wrong_extension_and_empty_bytes_do_not_create_artifacts() {
        let dir = tempfile::tempdir().unwrap();
        assert!(publish(dir.path(), Some("result.png"), "image", "jpg", b"jpeg", None, "generate_image").is_err());
        assert!(publish(dir.path(), Some("result.wav"), "audio", "wav", b"", None, "tts").is_err());
    ...
    fn generated_filenames_use_the_received_format() {
        let dir = tempfile::tempdir().unwrap();
        let path = publish(dir.path(), None, "images/generated", "jpg", b"jpeg", None, "generate_image").unwrap();
