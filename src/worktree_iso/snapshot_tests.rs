...
fn index_bytes(repo: &Path) -> Vec<u8> {
    ...
    fs::read(if index.is_absolute() { index } else { repo.join(index) }).unwrap()
}
...
fn dirty_snapshot_and_collection_preserve_both_real_indexes() {
    ...
    assert_eq!(fs::read(handle.path.join("base.txt")).unwrap(), b"working parent\n");
    assert_eq!(fs::read(handle.path.join("untracked.bin")).unwrap(), [0, 255, 1, 128]);
    ...
    assert_eq!(fs::read(repo.path().join("base.txt")).unwrap(), b"working child\n");
    assert_eq!(index_bytes(repo.path()), parent_index);
    assert_eq!(git_ok(repo.path(), &["rev-parse", "HEAD"]).unwrap(), parent_head);
...
fn tracked_ignored_additions_survive_without_copying_ignored_untracked_files() {
    ...
    fs::write(repo.path().join("private.ignored"), "not part of the snapshot\n").unwrap();
    git_ok(repo.path(), &["add", "-f", "tracked.ignored"]).unwrap();
    let before = index_bytes(repo.path());
    let handle = isolate(repo.path(), "ignored").unwrap();
    assert_eq!(fs::read(handle.path.join("tracked.ignored")).unwrap(), b"tracked\n");
...
fn unusual_untracked_names_and_binary_diffs_round_trip_without_loss() {
    ...
    for name in &names { fs::write(repo.path().join(name), &original).unwrap(); }
    ...
    apply_to_parent(&handle, &patch).unwrap();
    for name in &names { assert_eq!(fs::read(repo.path().join(name)).unwrap(), changed); }
    drop_worktree(&handle).unwrap();
}
...
fn symlinks_dangling_links_and_executable_modes_are_not_flattened() {
    ...
    fs::set_permissions(repo.path().join("run.sh"), fs::Permissions::from_mode(0o755)).unwrap();
    let handle = isolate(repo.path(), "file-types").unwrap();
    assert_eq!(fs::read_link(handle.path.join("alias")).unwrap(), Path::new("base.txt"));
    assert_eq!(fs::read_link(handle.path.join("dangling")).unwrap(), Path::new("missing-target"));
    assert_ne!(fs::metadata(handle.path.join("run.sh")).unwrap().permissions().mode() & 0o111, 0);
    assert!(collect_diff(&handle).unwrap().0.is_empty());
    drop_worktree(&handle).unwrap();
}
...
fn requesting_isolation_from_a_subdirectory_still_captures_the_whole_repo() {
    ...
    assert_eq!(fs::read(handle.path.join("outside.txt")).unwrap(), b"outside\n");
    assert_eq!(fs::read(handle.path.join("nested/inside.txt")).unwrap(), b"inside\n");
    drop_worktree(&handle).unwrap();
}
...
fn isolation_control_filenames_are_ordinary_user_files() {
    ...
    for name in names { fs::write(repo.path().join(name), "user-owned contents\n").unwrap(); }
    fs::write(repo.path().join("base.txt"), "parent dirty\n").unwrap();
    let handle = isolate(repo.path(), "control-names").unwrap();
    for name in names { assert_eq!(fs::read(handle.path.join(name)).unwrap(), b"user-owned contents\n"); }
    fs::write(handle.path.join("child.txt"), "child\n").unwrap();
    let (patch, _) = collect_diff(&handle).unwrap();
    assert!(!patch.contains(".pi-iso-"));
    apply_to_parent(&handle, &patch).unwrap();
    for name in names {
        assert_eq!(fs::read(repo.path().join(name)).unwrap(), b"user-owned contents\n");
        assert_eq!(fs::read(handle.path.join(name)).unwrap(), b"user-owned contents\n");
    }
    assert_eq!(collect_diff(&handle).unwrap().0, patch, "collection must not introduce its own patch file");
    drop_worktree(&handle).unwrap();
}
...
fn internal_baselines_do_not_require_user_identity_signing_or_commit_hooks() {
    ...
    git_ok(repo.path(), &["config", "core.hooksPath", hooks.path().to_str().unwrap()]).unwrap();
    ...
    git_ok(repo.path(), &["config", "gpg.program", "/not/a/real/signer"]).unwrap();
...
fn unsupported_submodules_and_sparse_checkouts_fail_before_creating_worktrees() {
    let repo = repository();
    let head = git_ok(repo.path(), &["rev-parse", "HEAD"]).unwrap();
    git_ok(repo.path(), &["update-index", "--add", "--cacheinfo", &format!("160000,{},vendor", head.trim())]).unwrap();
    ...
    assert!(error.to_string().contains("PI_ISO_SUBMODULE_UNSUPPORTED"), "{error}");
    assert_eq!(git_ok(repo.path(), &["worktree", "list", "--porcelain"]).unwrap(), before);
    ...
    assert!(error.to_string().contains("PI_ISO_SPARSE_UNSUPPORTED"), "{error}");
    assert_eq!(git_ok(sparse.path(), &["worktree", "list", "--porcelain"]).unwrap(), before);
}
...
fn a_linked_parent_worktree_keeps_its_own_index_and_head() {
    ...
    git_ok(repo.path(), &["worktree", "add", "-b", "linked-parent", linked.to_str().unwrap()]).unwrap();
    ...
    assert_eq!(fs::read(handle.path.join("base.txt")).unwrap(), b"linked actual\n");
    assert_eq!(index_bytes(&linked), index);
    assert_eq!(git_ok(&linked, &["rev-parse", "HEAD"]).unwrap(), head);
    assert_eq!(fs::read(repo.path().join("base.txt")).unwrap(), b"original\n");
    drop_worktree(&handle).unwrap();
    git_ok(repo.path(), &["worktree", "remove", "--force", linked.to_str().unwrap()]).unwrap();
}
...
fn non_utf8_text_diffs_fail_instead_of_replacing_bytes_lossily() {
    ...
    assert!(error.to_string().contains("PI_ISO_TEXT_ENCODING"), "{error}");
    assert_eq!(fs::read(repo.path().join("base.txt")).unwrap(), b"original\n");
    assert_eq!(fs::read(handle.path.join("base.txt")).unwrap(), [b'a', 255, b'\n']);
    drop_worktree(&handle).unwrap();
}
...
fn foreign_branch_in_a_prefix_named_directory_is_not_reaped() {
    ...
    git_ok(repo.path(), &["worktree", "add", "-b", "user-owned", foreign.to_str().unwrap()]).unwrap();
    ...
    git_ok(repo.path(), &["worktree", "remove", "--force", foreign.to_str().unwrap()]).unwrap();
}
