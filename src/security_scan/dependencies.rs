...
fn error(message: impl Into<String>) -> Error {
    Error::tool("security_scan", format!("[DEPENDENCY_INVENTORY] {}", message.into()))
}

impl Builder {
    fn entry(&mut self) -> Result<()> {
        self.entries += 1;
        if self.entries > MAX_ENTRIES { return Err(error("lockfile entries exceed 20000")); }
        Ok(())
    }

    fn exclude(&mut self, location: Location, reason: &str) {
        self.excluded.push(Excluded { location, reason: reason.to_string() });
    }

    fn add(&mut self, ecosystem: &str, name: &str, version: &str, location: Location) -> Result<()> {
        if !valid_name(ecosystem, name) || version.len() > 256 || semver::Version::parse(version).is_err() {
    ...
    fn finish(self, lockfiles: Vec<Lockfile>) -> Inventory {
        let packages = self.packages.into_iter().map(|((ecosystem, name, version), locations)| {
            Package { ecosystem, name, version, locations: locations.into_iter().collect() }
        }).collect();
        Inventory {
...
fn valid_name(ecosystem: &str, name: &str) -> bool {
    ...
    if ecosystem == "crates.io" {
        return name.bytes().all(|b| b.is_ascii_alphanumeric() || b"_-".contains(&b));
    }
    if let Some(scoped) = name.strip_prefix('@') {
        return scoped.split_once('/').is_some_and(|(scope, package)| component(scope) && component(package));
    }
    component(name)
}

fn normalized_path(raw: &str) -> Result<String> {
    if raw.is_empty() || raw.len() > 4096 || raw.contains('\\') || raw.contains(':') || raw.chars().any(char::is_control) {
        return Err(error("lockfile paths must be relative, NUL-free forward-slash paths"));
    }
    let mut parts = Vec::new();
    for component in Path::new(raw).components() {
        match component {
            Component::Normal(part) => parts.push(part.to_str().ok_or_else(|| error("lockfile path must be UTF-8"))?),
            Component::CurDir => {}
            _ => return Err(error("lockfile path cannot escape the project")),
        }
    }
    if parts.is_empty() { return Err(error("expected a lockfile, not a directory")); }
...
fn format_for(path: &str) -> Result<&'static str> {
    match Path::new(path).file_name().and_then(|name| name.to_str()) {
        ...
        _ => Err(error("supported lockfiles are Cargo.lock, package-lock.json and npm-shrinkwrap.json")),
    }
}
...
pub fn inventory(cwd: &Path, paths: &[String]) -> Result<Inventory> {
    if paths.len() > MAX_LOCKFILES { return Err(error("at most 32 lockfiles may be selected")); }
    let defaults = paths.is_empty();
    let selected = if defaults {
        vec!["Cargo.lock".to_string(), "package-lock.json".to_string()]
    } else { paths.to_vec() };
    let selected = selected.iter().map(|path| normalized_path(path)).collect::<Result<BTreeSet<_>>>()?;
    ...
    for path in selected {
        let bytes = match root.read(&path) {
            ...
            Err(failure) => return Err(error(format!("cannot read selected lockfile {path}: {failure}"))),
        };
        total += bytes.len();
        if total > MAX_TOTAL_BYTES { return Err(error("selected lockfiles exceed 16 MiB combined")); }
        let text = std::str::from_utf8(&bytes).map_err(|_| error(format!("lockfile {path} is not UTF-8")))?;
        ...
        lockfiles.push(Lockfile { path, format: format.to_string(), sha256: digest(&bytes) });
    }
    if lockfiles.is_empty() { return Err(error("no supported root lockfiles found; supply explicit paths for nested projects")); }
...
#[derive(Deserialize)]
struct CargoPackage { name: String, version: String, source: Option<String> }
...
fn parse_cargo(text: &str, path: &str, builder: &mut Builder) -> Result<()> {
    ...
    let parsed: CargoLock = toml::from_str(text).map_err(|_| error(format!("malformed Cargo lockfile {path}")))?;
    if parsed.version.is_some_and(|version| !(1..=4).contains(&version)) {
        return Err(error("unsupported Cargo lockfile version"));
    }
    for package in parsed.package {
        builder.entry()?;
        let location = Location { lockfile: path.to_string(), package_path: None };
...
fn parse_npm(text: &str, path: &str, builder: &mut Builder) -> Result<()> {
    let parsed: Value = serde_json::from_str(text).map_err(|_| error(format!("malformed npm lockfile {path}")))?;
    if !matches!(parsed["lockfileVersion"].as_u64(), Some(2 | 3)) {
        return Err(error("npm lockfileVersion must be 2 or 3; legacy v1 is not silently approximated"));
    }
    let packages = parsed["packages"].as_object().ok_or_else(|| error("npm lockfile has no packages map"))?;
    for (package_path, package) in packages {
        if package_path.is_empty() { continue; } // root project, not a dependency
        builder.entry()?;
        if package_path.len() > 4096 || package_path.chars().any(char::is_control) || !package.is_object() {
            return Err(error("invalid npm package location or descriptor"));
        }
        let location = Location { lockfile: path.to_string(), package_path: Some(package_path.clone()) };
        ...
        let installed_name = package_path.rsplit_once("node_modules/")
            .filter(|(prefix, _)| prefix.is_empty() || prefix.ends_with('/')).map(|(_, name)| name);
        ...
        let public = url::Url::parse(resolved).ok().is_some_and(|url| {
            url.scheme() == "https" && url.host_str() == Some("registry.npmjs.org")
...
impl Root {
    fn open(path: &Path) -> Result<Self> {
        ...
        rustix::fs::open(path, OFlags::RDONLY | OFlags::DIRECTORY | OFlags::CLOEXEC, Mode::empty())
            .map(Self).map_err(|_| error("cannot open project directory"))
    }
    fn read(&self, path: &str) -> std::io::Result<Vec<u8>> {
        ...
        while let Some(part) = parts.next() {
            ...
            if parts.peek().is_some() { flags |= OFlags::DIRECTORY; }
            let next = rustix::fs::openat(parent.as_ref().unwrap_or(&self.0), part, flags, Mode::empty())?;
            if parts.peek().is_none() {
                let file = std::fs::File::from(next);
                let metadata = file.metadata()?;
                if !metadata.is_file() || metadata.len() > MAX_FILE_BYTES {
                    return Err(std::io::Error::other("lockfile must be regular and at most 4 MiB"));
                }
...
impl Root {
    fn open(_path: &Path) -> Result<Self> {
        Err(error("confined dependency lockfile reads require a supported Unix descriptor backend"))
    }
    fn read(&self, _path: &str) -> std::io::Result<Vec<u8>> {
        Err(std::io::Error::new(std::io::ErrorKind::Unsupported, "unsupported descriptor backend"))
    }
}
