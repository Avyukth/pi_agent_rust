...
fn authorization_override(options: &StreamOptions, compat: Option<&CompatConfig>) -> Option<String> {
    super::first_non_empty_header_value_case_insensitive(&options.headers, &["authorization"])
        .or_else(|| {
            compat.and_then(|compat| compat.custom_headers.as_ref()).and_then(|headers| {
                super::first_non_empty_header_value_case_insensitive(headers, &["authorization"])
            })
        })
}

fn response_secrets(options: &StreamOptions, compat: Option<&CompatConfig>, fallback: Option<&str>) -> Vec<String> {
    let mut secrets = fallback.into_iter().map(str::to_string).collect::<Vec<_>>();
    let headers = options.headers.iter().chain(
        compat.and_then(|compat| compat.custom_headers.as_ref()).into_iter().flatten(),
    );
...
impl Provider for CohereProvider {
    ...
    fn api(&self) -> &'static str { "cohere-chat" }
    ...
    async fn stream(
        ...
        let mut request = self.client.post(&self.base_url).header("Accept", "text/event-stream");
            ...
            && let Some(custom_headers) = &compat.custom_headers
        {
            request = super::apply_headers_ignoring_blank_auth_overrides(
                request, custom_headers, &["authorization"],
            );
        }
        request = super::apply_headers_ignoring_blank_auth_overrides(
        ...
        let rewritten_body = super::offer_before_provider_request(
            options, self.name(), self.api(), self.model_id(), &self.base_url, &request_body,
            |value| super::validate_streamed_json_rewrite(
                value, &["model"], &["messages"], &[("stream", serde_json::Value::Bool(true))],
            ),
        ).await;
        ...
        if !(200..300).contains(&status) {
            let body = response.text_limited(MAX_ERROR_BYTES).await
            ...
            return Err(Error::provider("cohere", format!("Cohere API error (HTTP {status}): {body}")));
        }
        let is_sse = response.headers().iter()
            .find(|(name, _)| name.eq_ignore_ascii_case("content-type"))
            .is_some_and(|(_, value)| value.split(';').next().is_some_and(|mime| mime.trim().eq_ignore_ascii_case("text/event-stream")));
        if !is_sse {
            return Err(Error::api(format!("Cohere API protocol error (HTTP {status}): expected Content-Type text/event-stream")));
        }
        Ok(streaming::response_stream(
            response.bytes_stream(), self.model.clone(), self.api().to_string(), self.name().to_string(),
        ))
    }
}
...
enum CohereMessage {
    ...
    User { content: request_options::InputContent },
    Assistant {
        #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Serialize)]
struct CohereFunctionRef { name: String, arguments: String }
...
fn convert_tool_to_cohere(tool: &ToolDef) -> CohereTool {
    CohereTool {
        ...
        function: CohereFunction {
            ...
            description: if tool.description.trim().is_empty() { None } else { Some(tool.description.clone()) },
...
fn build_cohere_messages(context: &Context<'_>) -> Vec<CohereMessage> {
    ...
    if let Some(system) = &context.system_prompt {
        out.push(CohereMessage::System { content: system.to_string() });
    }
    for message in context.messages.iter() {
        ...
        match message {
                ...
                for block in &assistant.content {
                    match block {
                        ...
                        ContentBlock::ToolCall(tc) => tool_calls.push(CohereToolCallRef {
                            id: tc.id.clone(), r#type: "function",
                            function: CohereFunctionRef { name: tc.name.clone(), arguments: tc.arguments.to_string() },
                        }),
                        _ => {}
                    }
                }
            ...
            Message::ToolResult(result) => {
                ...
                let mut content = result.content.iter().filter_map(|block| match block {
                ...
                out.push(CohereMessage::Tool { content, tool_call_id: result.tool_call_id.clone() });
            }
        }
    }
    flush_tool_images(&mut out, &mut pending_images);
...
mod tests {
    ...
    struct ProviderFixture { cases: Vec<ProviderCase> }

    #[derive(Debug, Deserialize)]
    struct ProviderCase { name: String, events: Vec<Value>, expected: Vec<EventSummary> }

    #[derive(Debug, Deserialize, Serialize, PartialEq)]
    struct EventSummary {
        kind: String,
        #[serde(default)] content_index: Option<usize>,
        #[serde(default)] delta: Option<String>,
    ...
    fn load_fixture(file_name: &str) -> ProviderFixture {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/provider_responses").join(file_name);
    ...
    fn summarize_event(event: &StreamEvent) -> EventSummary {
        let mut summary = EventSummary { kind: "other".to_string(), content_index: None, delta: None, content: None, reason: None };
        match event {
            StreamEvent::Start { .. } => summary.kind = "start".to_string(),
            StreamEvent::TextStart { content_index } => {
                summary.kind = "text_start".to_string(); summary.content_index = Some(*content_index);
            }
            StreamEvent::TextDelta { content_index, delta } => {
                summary.kind = "text_delta".to_string(); summary.content_index = Some(*content_index); summary.delta = Some(delta.clone());
            }
            StreamEvent::TextEnd { content_index, content } => {
                summary.kind = "text_end".to_string(); summary.content_index = Some(*content_index); summary.content = Some(content.clone());
            }
            StreamEvent::Done { reason, .. } => {
                summary.kind = "done".to_string(); summary.reason = Some(reason_to_string(*reason));
            }
            StreamEvent::Error { reason, .. } => {
                summary.kind = "error".to_string(); summary.reason = Some(reason_to_string(*reason));
            }
            _ => {}
        }
        summary
    ...
    fn reason_to_string(reason: StopReason) -> String {
        match reason {
            StopReason::Stop => "stop", StopReason::Length => "length", StopReason::ToolUse => "tool_use",
            StopReason::PauseTurn => "pause_turn", StopReason::Refusal => "refusal", StopReason::Error => "error",
    ...
    fn test_build_request_includes_system_tools_and_v2_shape() {
        ...
        let context = Context::owned(
            ...
            vec![Message::User(crate::model::UserMessage { content: UserContent::Text("Ping".to_string()), timestamp: 0 })],
            vec![ToolDef {
                name: "search".to_string(), description: "Search docs".to_string(),
                parameters: json!({"type":"object","properties":{"q":{"type":"string"}},"required":["q"]}),
            }],
        );
        let options = StreamOptions { temperature: Some(0.2), max_tokens: Some(123), ..Default::default() };
        ...
        assert_eq!(value["tools"][0]["function"]["parameters"], json!({"type":"object","properties":{"q":{"type":"string"}},"required":["q"]}));
    }

    #[test]
    fn test_convert_tool_to_cohere_omits_empty_description() {
        let tool = ToolDef { name: "echo".to_string(), description: "   ".to_string(), parameters: json!({"type":"object","properties":{"text":{"type":"string"}}}) };
    ...
    fn test_stream_parses_thinking_and_max_tokens_stop_reason() {
        ...
        let out = collect_events(&events);
        assert!(out.iter().any(|e| matches!(e, StreamEvent::ThinkingStart { .. })));
        assert!(out.iter().any(|e| matches!(e, StreamEvent::ThinkingDelta { delta, .. } if delta.contains("Plan"))));
        assert!(out.iter().any(|e| matches!(e, StreamEvent::ThinkingEnd { content, .. } if content.contains("Plan more"))));
        assert!(out.iter().any(|e| matches!(e, StreamEvent::Done { reason: StopReason::Length, .. })));
    }

    #[test]
    fn test_stream_sets_bearer_auth_header() {
        let captured = run_stream_and_capture_headers(Some("test-cohere-key"), HashMap::new()).expect("captured request");
        assert_eq!(captured.headers.get("authorization").map(String::as_str), Some("Bearer test-cohere-key"));
    ...
    fn test_stream_uses_existing_authorization_header_without_api_key() {
        ...
        headers.insert("Authorization".to_string(), "Bearer from-custom-header".to_string());
        headers.insert("X-Test".to_string(), "1".to_string());
        let captured = run_stream_and_capture_headers(None, headers).expect("captured request");
        assert_eq!(captured.headers.get("authorization").map(String::as_str), Some("Bearer from-custom-header"));
    ...
    fn test_stream_compat_authorization_header_overrides_api_key_without_duplicate() {
        ...
        custom_headers.insert("Authorization".to_string(), "Bearer compat-header".to_string());
        let provider = CohereProvider::new("command-r").with_base_url(base_url).with_compat(Some(CompatConfig { custom_headers: Some(custom_headers), ..Default::default() }));
        let context = Context::owned(Some("system".to_string()), vec![Message::User(crate::model::UserMessage { content: UserContent::Text("ping".to_string()), timestamp: 0 })], Vec::new());
        let options = StreamOptions { api_key: Some("test-cohere-key".to_string()), ..Default::default() };
        let runtime = RuntimeBuilder::current_thread().build().unwrap();
        runtime.block_on(async {
            let mut stream = provider.stream(&context, &options).await.unwrap();
            while let Some(event) = stream.next().await {
                if matches!(event.unwrap(), StreamEvent::Done { .. }) { break; }
            }
        });
        let captured = rx.recv_timeout(Duration::from_secs(2)).unwrap();
        assert_eq!(captured.headers.get("authorization").map(String::as_str), Some("Bearer compat-header"));
    ...
    fn test_stream_compat_authorization_header_works_without_api_key() {
        ...
        let runtime = RuntimeBuilder::current_thread().build().unwrap();
        runtime.block_on(async {
            let mut stream = provider.stream(&context, &StreamOptions::default()).await.unwrap();
            while let Some(event) = stream.next().await {
                if matches!(event.unwrap(), StreamEvent::Done { .. }) { break; }
            }
        });
    ...
    fn collect_events(events: &[Value]) -> Vec<StreamEvent> {
        ...
        runtime.block_on(async {
            let byte_stream = stream::iter(events.iter().map(|event| format!("data: {}\n\n", serde_json::to_string(event).unwrap())).map(|s| Ok(s.into_bytes())));
            let event_source = crate::sse::SseStream::new(Box::pin(byte_stream));
            let mut state = StreamState::new(event_source, "command-r".to_string(), "cohere-chat".to_string(), "cohere".to_string());
            let mut out = Vec::new();
    ...
    impl CapturedRequest {
        fn header_count(&self, name: &str) -> usize {
            self.header_lines.iter().filter(|(key, _)| key.eq_ignore_ascii_case(name)).count()
        }
    }

    fn run_stream_and_capture_headers(api_key: Option<&str>, extra_headers: HashMap<String, String>) -> Option<CapturedRequest> {
        let (base_url, rx) = spawn_test_server(200, "text/event-stream", &success_sse_body());
        let provider = CohereProvider::new("command-r").with_base_url(base_url);
        let context = Context::owned(Some("system".to_string()), vec![Message::User(crate::model::UserMessage { content: UserContent::Text("ping".to_string()), timestamp: 0 })], Vec::new());
        let options = StreamOptions { api_key: api_key.map(str::to_string), headers: extra_headers, ..Default::default() };
        let runtime = RuntimeBuilder::current_thread().build().expect("runtime build");
        runtime.block_on(async {
            let mut stream = provider.stream(&context, &options).await.unwrap();
            while let Some(event) = stream.next().await {
    ...
    fn spawn_test_server(status_code: u16, content_type: &str, body: &str) -> (String, mpsc::Receiver<CapturedRequest>) {
        ...
        std::thread::spawn(move || {
            ...
            tx.send(CapturedRequest { headers, header_lines, body: String::from_utf8_lossy(&request_body).to_string() }).expect("send captured request");
            let reason = match status_code { 401 => "Unauthorized", 500 => "Internal Server Error", _ => "OK" };
    ...
    fn test_build_request_no_system_prompt() {
        ...
        let context = Context::owned(None, vec![Message::User(crate::model::UserMessage { content: UserContent::Text("Hi".to_string()), timestamp: 0 })], vec![]);
        let value = serde_json::to_value(provider.build_request(&context, &StreamOptions::default())).unwrap();
        assert_eq!(value["messages"][0]["role"], "user");
        assert_eq!(value["messages"][0]["content"], "Hi");
        assert!(!value["messages"].as_array().unwrap().iter().any(|m| m["role"] == "system"));
    }

    #[test]
    fn test_build_request_default_max_tokens() {
        let provider = CohereProvider::new("command-r");
        let context = Context::owned(None, vec![Message::User(crate::model::UserMessage { content: UserContent::Text("test".to_string()), timestamp: 0 })], vec![]);
        let value = serde_json::to_value(provider.build_request(&context, &StreamOptions::default())).unwrap();
    ...
    fn test_build_request_full_conversation_with_tool_call_and_result() {
        let provider = CohereProvider::new("command-r");
        let context = Context::owned(Some("Be concise.".to_string()), vec![
            Message::User(crate::model::UserMessage { content: UserContent::Text("Read /tmp/a.txt".to_string()), timestamp: 0 }),
            Message::assistant(AssistantMessage {
                content: vec![ContentBlock::ToolCall(ToolCall { id: "call_1".to_string(), name: "read".to_string(), arguments: json!({"path":"/tmp/a.txt"}), thought_signature: None })],
                api: "cohere-chat".to_string(), provider: "cohere".to_string(), model: "command-r".to_string(), usage: Usage::default(), stop_reason: StopReason::ToolUse, stop_details: None, error_message: None, timestamp: 1,
            }),
            Message::tool_result(crate::model::ToolResultMessage { tool_call_id: "call_1".to_string(), tool_name: "read".to_string(), content: vec![ContentBlock::Text(TextContent::new("file contents"))], details: None, is_error: false, timestamp: 2 }),
        ], vec![ToolDef { name: "read".to_string(), description: "Read a file".to_string(), parameters: json!({"type":"object"}) }]);
        let value = serde_json::to_value(provider.build_request(&context, &StreamOptions::default())).unwrap();
        let msgs = value["messages"].as_array().unwrap();
        assert_eq!(msgs.len(), 4);
        assert_eq!(msgs[0]["role"], "system"); assert_eq!(msgs[1]["role"], "user");
        assert_eq!(msgs[2]["role"], "assistant"); assert_eq!(msgs[3]["role"], "tool");
        assert!(msgs[2].get("content").is_none() || msgs[2]["content"].is_null());
        let calls = msgs[2]["tool_calls"].as_array().unwrap();
        assert_eq!(calls.len(), 1); assert_eq!(calls[0]["id"], "call_1");
        assert_eq!(calls[0]["type"], "function"); assert_eq!(calls[0]["function"]["name"], "read");
        assert_eq!(msgs[3]["tool_call_id"], "call_1"); assert_eq!(msgs[3]["content"], "file contents");
    }

    #[test]
    fn test_build_request_assistant_text_preserved_alongside_tool_calls() {
        let provider = CohereProvider::new("command-r");
        let context = Context::owned(None, vec![Message::assistant(AssistantMessage {
            content: vec![ContentBlock::Text(TextContent::new("Let me read that file.")), ContentBlock::ToolCall(ToolCall { id: "call_1".to_string(), name: "read".to_string(), arguments: json!({"path":"/tmp/a.txt"}), thought_signature: None })],
            api: "cohere-chat".to_string(), provider: "cohere".to_string(), model: "command-r".to_string(), usage: Usage::default(), stop_reason: StopReason::ToolUse, stop_details: None, error_message: None, timestamp: 0,
        })], vec![]);
        let value = serde_json::to_value(provider.build_request(&context, &StreamOptions::default())).unwrap();
        assert_eq!(value["messages"][0]["role"], "assistant");
        assert_eq!(value["messages"][0]["content"].as_str(), Some("Let me read that file."));
        assert_eq!(value["messages"][0]["tool_calls"].as_array().unwrap().len(), 1);
    }

    #[test]
    fn test_convert_custom_message_to_cohere() {
        let context = Context::owned(None, vec![Message::Custom(crate::model::CustomMessage { custom_type: "extension_note".to_string(), content: "Important context.".to_string(), display: false, details: None, timestamp: 0 })], vec![]);
        let msgs = build_cohere_messages(&context);
        assert_eq!(msgs.len(), 1);
        let value = serde_json::to_value(&msgs[0]).unwrap();
        assert_eq!(value["role"], "user"); assert_eq!(value["content"], "Important context.");
    }

    #[test]
    fn test_convert_user_blocks_extracts_text_only() {
        let content = UserContent::Blocks(vec![ContentBlock::Text(TextContent::new("part 1")), ContentBlock::Image(crate::model::ImageContent { data: "aGVsbG8=".to_string(), mime_type: "image/png".to_string() }), ContentBlock::Text(TextContent::new("part 2"))]);
        assert_eq!(extract_text_user_content(&content), "part 1[Image: image/png (8 bytes)]part 2");
        ...
        assert_eq!(native[1]["image_url"]["url"], "data:image/png;base64,aGVsbG8=");
    }

    #[test]
    fn test_custom_provider_name() {
        let provider = CohereProvider::new("command-r").with_provider_name("my-proxy");
        assert_eq!(provider.name(), "my-proxy"); assert_eq!(provider.api(), "cohere-chat");
    }

    #[test]
    fn test_custom_base_url() {
        let provider = CohereProvider::new("command-r").with_base_url("https://proxy.example.com/v2/chat");
    ...
    fn test_stream_error_finish_reason_maps_to_error() {
        ...
        assert!(out.iter().any(|e| matches!(e, StreamEvent::Done { reason: StopReason::Error, .. })));
    }
    ...
    fn test_stream_tool_call_with_streamed_arguments() {
        ...
        let call = out.iter().find_map(|e| match e { StreamEvent::ToolCallEnd { tool_call, .. } => Some(tool_call), _ => None }).expect("ToolCallEnd");
        assert_eq!(call.name, "bash"); assert_eq!(call.id, "call_42"); assert_eq!(call.arguments["command"], "ls -la");
    }

    #[test]
    fn test_stream_unknown_event_type_ignored() {
        let out = collect_events(&[
            json!({"type":"message-start","id":"msg_1"}), json!({"type":"some-future-event","data":"ignored"}),
        ...
        assert!(out.iter().any(|e| matches!(e, StreamEvent::TextStart { content_index: 0, .. })));
    }
}
...
pub mod fuzz {
    ...
    type FuzzStream = Pin<Box<futures::stream::Empty<std::result::Result<Vec<u8>, std::io::Error>>>>;
    ...
    impl Processor {
        ...
        pub fn new() -> Self {
            ...
            Self(StreamState::new(crate::sse::SseStream::new(Box::pin(empty)), "cohere-fuzz".into(), "cohere".into(), "cohere".into()))
        }
