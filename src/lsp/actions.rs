...
use super::{LspInput, LspTool, display_path, resolve_tool_path, select_code_action, text_output, tool_err};
...
impl CommandLease {
    fn finish(self) -> Report {
        ...
        let report = if active.as_ref().is_some_and(|grant| grant.id == self.id) {
            active.take().map_or_else(Report::default, |grant| grant.report)
        } else {
            Report::default()
        };
        drop(active);
...
impl ActionState {
    fn grant(self: &Arc<Self>, entry: &Arc<ServerEntry>) -> Result<CommandLease> {
        ...
        if active.is_some() {
            return Err(tool_err("LSP_EDIT_BUSY", "another code action owns the edit window"));
        }
        ...
        *active = Some(Grant {
        ...
        Ok(CommandLease { state: Arc::clone(self), id })
    }
    ...
    fn apply_from_server(&self, entry: &Arc<ServerEntry>, params: &Value) -> Value {
        ...
        let outcome = if grant.report.requests > MAX_APPLY_REQUESTS {
            Err(tool_err("LSP_EDIT_LIMIT", "code action exceeded its server-edit request budget"))
        } else {
            params.get("edit")
    ...
    fn remember(&self, entry: &Arc<ServerEntry>, path: &Path, hash: u64, action: Value) -> Result<String> {
        let bytes = serde_json::to_vec(&action)?.len();
        if bytes > MAX_ACTION_BYTES {
            return Err(tool_err("LSP_ACTION_LIMIT", "one code action exceeds 2 MiB"));
        }
        ...
        cache.push_back(CachedAction {
            id: id.clone(), entry: Arc::downgrade(entry), path: path.to_path_buf(), hash, action, bytes,
        });
        Ok(id)
    }
    ...
    fn take(&self, id: &str) -> Result<CachedAction> {
        let mut cache = lock(&self.cache);
        let index = cache.iter().position(|item| item.id == id)
            .ok_or_else(|| tool_err("LSP_ACTION_EXPIRED", "unknown or expired actionId; list code actions again"))?;
        cache.remove(index).ok_or_else(|| tool_err("LSP_ACTION_EXPIRED", "code action expired"))
    }
}
...
pub(super) fn install_apply_edit_handler(entry: &Arc<ServerEntry>, state: Arc<ActionState>) {
    ...
    entry.client.set_server_request_handler(Arc::new(move |method, params| {
...
fn file_hash(path: &Path) -> Result<u64> {
    ...
    if !file.metadata()?.is_file() {
        return Err(tool_err("LSP_FILE_UNREADABLE", "code action source is not a regular file"));
    }
    let mut content = String::new();
    use std::io::Read as _;
    file.take(16 * 1024 * 1024 + 1).read_to_string(&mut content)?;
    if content.len() > 16 * 1024 * 1024 {
        return Err(tool_err("LSP_ACTION_LIMIT", "code action source exceeds 16 MiB"));
    }
...
fn verify_source(path: &Path, expected: u64) -> Result<()> {
    if file_hash(path)? != expected {
        return Err(tool_err("LSP_EDIT_CONFLICT", "source changed since code actions were requested; list again"));
    }
    Ok(())
}
...
fn inside_root(path: &Path, root: &Path) -> Result<()> {
    if !path.is_absolute() || path.components().any(|part| part == std::path::Component::ParentDir) {
        return Err(tool_err("LSP_EDIT_SCOPE", "server edits require absolute local file paths"));
    }
    for ancestor in path.ancestors() {
        match std::fs::symlink_metadata(ancestor) {
            Ok(metadata) if metadata.file_type().is_symlink() => {
                return Err(tool_err("LSP_EDIT_SCOPE", "symlink components are not allowed in server edit paths"));
            }
            Ok(_) => {}
    ...
    while !prefix.try_exists()? {
        suffix.push(prefix.file_name().ok_or_else(|| tool_err("LSP_EDIT_SCOPE", "invalid edit path"))?);
        prefix = prefix.parent().ok_or_else(|| tool_err("LSP_EDIT_SCOPE", "edit path has no existing parent"))?;
    }
    ...
    if !resolved.starts_with(root) || resolved == root {
        return Err(tool_err("LSP_EDIT_SCOPE", "server edit escapes the language server workspace"));
    }
    Ok(())
}

fn apply_scoped(entry: &ServerEntry, edit: &Value, hashes: Option<&HashMap<PathBuf, u64>>) -> Result<ApplyOutcome> {
...
fn enabled(action: &Value) -> Result<()> {
    if !action.is_object() || action["title"].as_str().is_none_or(str::is_empty) {
        return Err(tool_err("LSP_ACTION_MALFORMED", "code action needs a nonempty title"));
    }
    if let Some(disabled) = action.get("disabled").filter(|value| !value.is_null()) {
        return Err(tool_err("LSP_ACTION_DISABLED", disabled["reason"].as_str().unwrap_or("server disabled this code action")));
    }
    Ok(())
}
...
fn command(action: &Value) -> Result<Option<Value>> {
    ...
    let (name, arguments) = if let Some(name) = command.as_str() {
        ...
        (command["command"].as_str().ok_or_else(|| tool_err("LSP_ACTION_MALFORMED", "command object has no command name"))?, command.get("arguments"))
    };
    if name.is_empty() || name.len() > 1024 || name.contains('\0') {
        return Err(tool_err("LSP_ACTION_MALFORMED", "invalid code action command name"));
    }
    let mut params = json!({"command":name});
    if let Some(arguments) = arguments.filter(|value| !value.is_null()) {
        if !arguments.is_array() {
            return Err(tool_err("LSP_ACTION_MALFORMED", "command arguments must be an array"));
        }
...
fn validate_resolution(original: &Value, resolved: Value) -> Result<Value> {
    ...
    for (key, value) in original.as_object().ok_or_else(|| tool_err("LSP_ACTION_MALFORMED", "invalid selected action"))? {
        if resolved.get(key) != Some(value) {
            return Err(tool_err("LSP_ACTION_CHANGED", format!("codeAction/resolve changed existing property {key}")));
        }
    }
    Ok(resolved)
}
    ...
    pub(super) async fn run_code_actions(&self, input: &LspInput) -> Result<ToolOutput> {
        let (path, entry, hash, selected) = if let Some(id) = input.action_id.as_deref() {
            if !input.apply.unwrap_or(false) || input.query.is_some() {
                return Err(tool_err("LSP_USAGE", "actionId requires apply:true and cannot be combined with query"));
            }
            let cached = self.actions.take(id)?;
            let entry = cached.entry.upgrade().filter(|entry| entry.client.is_alive())
                .ok_or_else(|| tool_err("LSP_ACTION_EXPIRED", "language server restarted; list code actions again"))?;
            ...
            let file = input.file.as_deref().ok_or_else(|| tool_err("LSP_USAGE", "code_actions requires file or a cached actionId"))?;
            ...
            let diagnostics = entry.client.diagnostics_snapshot().get(&uri).cloned().unwrap_or_default();
            let result = entry.client.call("textDocument/codeAction", json!({
                "textDocument":{"uri":uri},"range":range,
                "context":{"diagnostics":diagnostics}
            }), self.request_timeout(input)).await?;
            verify_source(&path, hash)?;
            let actions = if result.is_null() { Vec::new() } else {
                result.as_array().cloned().ok_or_else(|| tool_err("LSP_ACTION_MALFORMED", "codeAction result must be an array or null"))?
            };
            if actions.len() > MAX_ACTIONS || serde_json::to_vec(&actions)?.len() > MAX_ACTION_BYTES {
                return Err(tool_err("LSP_ACTION_LIMIT", "code action response exceeds 128 actions or 2 MiB"));
            }
            if !input.apply.unwrap_or(false) {
                ...
                for (index, action) in actions.iter().enumerate() {
                    let title = action["title"].as_str().filter(|title| !title.is_empty())
                        .ok_or_else(|| tool_err("LSP_ACTION_MALFORMED", "code action has no title"))?;
            ...
            let query = input.query.as_deref().filter(|query| !query.trim().is_empty())
                .ok_or_else(|| tool_err("LSP_USAGE", "apply:true requires actionId or a nonempty query"))?;
        ...
        let selected = if selected["command"].is_string() {
            ...
            selected
        } else if entry.client.capabilities().raw.pointer("/codeActionProvider/resolveProvider") == Some(&Value::Bool(true))
            && selected.get("edit").is_none_or(Value::is_null)
        {
            let resolved = entry.client.call("codeAction/resolve", selected.clone(), self.request_timeout(input)).await?;
        ...
        if edit.is_none() && command.is_none() {
            return Err(tool_err("LSP_ACTION_NO_EDIT", "resolved code action contains neither an edit nor a command"));
        }
        verify_source(&path, hash)?;
        ...
        if let Some(params) = &command {
            ...
            let result = entry.client.call("workspace/executeCommand", params.clone(), self.request_timeout(input)).await;
        ...
        let files: Vec<_> = report.files.iter().map(|path| display_path(path, &self.cwd)).collect();
        let payload = json!({
...
mod tests {
    ...
    fn disabled_and_changed_resolved_actions_are_rejected() {
        ...
        assert!(validate_resolution(&action,json!({"title":"Other","data":{"id":7},"edit":{}})).is_err());
        assert!(validate_resolution(&action,json!({"title":"Fix","data":{"id":7},"edit":{}})).is_ok());
    }

    #[test]
    fn commands_preserve_literal_arguments_and_reject_malformed_shapes() {
        assert_eq!(command(&json!({"title":"Fix","command":"fix","arguments":["$(literal)"]})).unwrap(),
            Some(json!({"command":"fix","arguments":["$(literal)"]})));
        assert_eq!(command(&json!({"title":"Fix","command":{"command":"fix","arguments":[7]}})).unwrap(),
            Some(json!({"command":"fix","arguments":[7]})));
        assert!(command(&json!({"title":"Fix","command":{"command":"fix","arguments":{}}})).is_err());
    }
