...
fn frame(tool: &DebugTool, runtime: &asupersync::runtime::Runtime, thread: u64) -> u64 {
    run(tool, runtime, json!({"action":"stack_trace","threadId":thread})).unwrap()["frames"][0]["id"].as_u64().unwrap()
}
...
fn object(tool: &DebugTool, runtime: &asupersync::runtime::Runtime, thread: u64) -> u64 {
    run(tool, runtime, json!({"action":"evaluate","expression":"value","threadId":thread})).unwrap()["variablesReference"].as_u64().unwrap()
}

#[cfg(unix)]
fn capture(tool: &DebugTool, runtime: &asupersync::runtime::Runtime) -> Vec<Value> {
    run(tool, runtime, json!({"action":"custom_request","command":"capture"})).unwrap()["result"]["requests"].as_array().unwrap().clone()
}
...
fn unsupported_configuration_done_is_not_sent() {
    ...
    let runtime = runtime();
    launch(&tool, &runtime);
    assert!(!capture(&tool, &runtime).iter().any(|request| request["command"] == "configurationDone"));
    run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
}
...
fn partial_resume_preserves_peer_handles_and_pause_targets_the_running_thread() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(), "thread_pair") else { return; };
    let runtime = runtime();
    launch(&tool, &runtime);
    ...
    let resumed = run(&tool, &runtime, json!({"action":"continue","threadId":7,"singleThread":true})).unwrap();
    let states = resumed["execution"]["threads"].as_array().unwrap();
    assert_eq!(states.iter().find(|thread| thread["id"] == 7).unwrap()["state"], "running");
    assert_eq!(states.iter().find(|thread| thread["id"] == 8).unwrap()["state"], "stopped");
    for input in [json!({"action":"scopes","frameId":first_frame}),
        json!({"action":"variables","variablesReference":first_object})]
    {
        assert!(run(&tool, &runtime, input).unwrap_err().to_string().contains("DAP_STALE_REFERENCE"));
    }
    run(&tool, &runtime, json!({"action":"scopes","frameId":peer_frame,"threadId":8})).unwrap();
    run(&tool, &runtime, json!({"action":"variables","variablesReference":peer_object,"threadId":8})).unwrap();
...
fn single_thread_step_waits_for_the_requested_thread_and_rejects_recycled_handles() {
    for mode in ["thread_pair", "thread_pair_before_reply"] {
        ...
        assert_ne!(new_frame, old_frame, "adapter frame ID 21 is deliberately recycled");
        assert_ne!(new_object, old_object, "adapter object ID 66 is deliberately recycled");
        assert!(run(&tool, &runtime, json!({"action":"scopes","frameId":old_frame})).is_err());
        assert!(run(&tool, &runtime, json!({"action":"variables","variablesReference":old_object})).is_err());
        run(&tool, &runtime, json!({"action":"scopes","frameId":peer_frame})).unwrap();
        let requests = capture(&tool, &runtime);
        let step = requests.iter().find(|request| request["command"] == "next").unwrap();
        assert_eq!(step["arguments"], json!({"threadId":7,"singleThread":true,"granularity":"instruction"}));
        run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
    }
}
...
fn rejected_resume_restores_selected_thread_without_resurrecting_a_resumed_peer() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(), "resume_rejected") else { return; };
    let runtime = runtime();
    launch(&tool, &runtime);
    run(&tool, &runtime, json!({"action":"threads"})).unwrap();
    let first_frame = frame(&tool, &runtime, 7);
    let peer_frame = frame(&tool, &runtime, 8);
    let error = run(&tool, &runtime, json!({"action":"continue","threadId":7,"singleThread":true})).unwrap_err();
    assert!(error.to_string().contains("resume rejected"));
    run(&tool, &runtime, json!({"action":"scopes","frameId":first_frame})).unwrap();
    assert!(run(&tool, &runtime, json!({"action":"scopes","frameId":peer_frame})).is_err());
    assert!(run(&tool, &runtime, json!({"action":"stack_trace","threadId":8})).unwrap_err().to_string().contains("DAP_STATE_RUNNING"));
    run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
}
...
fn a_running_thread_cannot_borrow_a_stop_or_another_threads_frame() {
    ...
    let first_frame = frame(&tool, &runtime, 7);
    assert!(run(&tool, &runtime, json!({"action":"stack_trace","threadId":8})).unwrap_err().to_string().contains("DAP_STATE_RUNNING"));
    assert!(run(&tool, &runtime, json!({"action":"evaluate","expression":"count","frameId":first_frame,"threadId":8})).is_err());
    assert!(run(&tool, &runtime, json!({"action":"variables","variablesReference":first_frame})).unwrap_err().to_string().contains("DAP_STALE_REFERENCE"));
    let requests = capture(&tool, &runtime);
    assert!(!requests.iter().any(|request| request["command"] == "evaluate" || request["command"] == "variables"));
    assert!(!requests.iter().any(|request| request["command"] == "stackTrace" && request["arguments"]["threadId"] == 8));
    run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
}
...
fn assignment_roundtrip_refreshes_objects_and_retains_exception_causes() {
    ...
    assert!(run(&tool, &runtime, json!({"action":"variables","variablesReference":parent})).unwrap_err().to_string().contains("DAP_STALE_REFERENCE"));
    let value = run(&tool, &runtime, json!({"action":"evaluate","frameId":first_frame,"expression":"count"})).unwrap();
    ...
    let exception = run(&tool, &runtime, json!({"action":"exception_info","threadId":7})).unwrap();
    assert_eq!(exception["exceptionId"], "ValueError");
    assert_eq!(exception["details"]["stackTrace"], "fixture stack");
    assert_eq!(exception["details"]["innerException"][0]["message"], "root cause");
    let requests = capture(&tool, &runtime);
    let set = requests.iter().find(|request| request["command"] == "setVariable").unwrap();
    assert_eq!(set["arguments"], json!({"variablesReference":41,"name":"count","value":"41"}));
    let set = requests.iter().find(|request| request["command"] == "setExpression").unwrap();
    assert_eq!(set["arguments"], json!({"frameId":21,"expression":"count","value":"99"}));
    run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
}
...
fn unsupported_advanced_operations_fail_before_adapter_dispatch() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(), "no_inspection_capabilities") else { return; };
    let runtime = runtime();
    launch(&tool, &runtime);
    for input in [
    ...
    let requests = capture(&tool, &runtime);
    assert!(!requests.iter().any(|request| matches!(request["command"].as_str(),
...
fn invalidation_or_resume_during_a_reply_does_not_publish_stale_inspection() {
    for mode in ["invalidate_during_variables", "resume_during_variables"] {
        ...
        let old_frame = frame(&tool, &runtime, 7);
        let old_object = object(&tool, &runtime, 7);
        let error = run(&tool, &runtime, json!({"action":"variables","variablesReference":old_object})).unwrap_err();
        assert!(error.to_string().contains("DAP_STALE_REFERENCE"), "{error}");
        assert!(run(&tool, &runtime, json!({"action":"variables","variablesReference":old_object})).is_err());
        assert!(run(&tool, &runtime, json!({"action":"scopes","frameId":old_frame})).is_err());
        assert_ne!(object(&tool, &runtime, 7), old_object);
        let requests = capture(&tool, &runtime);
        assert_eq!(requests.iter().filter(|request| request["command"] == "variables").count(), 1);
        run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
    }
}
...
fn malformed_assignment_response_does_not_restore_revoked_parent_or_claim_success() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(), "invalid_set_result") else { return; };
    let runtime = runtime();
    launch(&tool, &runtime);
    let first_frame = frame(&tool, &runtime, 7);
    let parent = locals(&tool, &runtime, first_frame);
    let error = run(&tool, &runtime, json!({"action":"set_variable","variablesReference":parent,"name":"count","value":"123"})).unwrap_err();
    assert!(error.to_string().contains("DAP_PROTOCOL"));
    assert!(run(&tool, &runtime, json!({"action":"variables","variablesReference":parent})).is_err());
...
fn frame_handles_cannot_cross_vendor_requests_or_session_restarts() {
    ...
    assert!(run(&tool, &runtime, json!({"action":"scopes","frameId":previous_session})).is_err());
    run(&tool, &runtime, json!({"action":"scopes","frameId":fresh})).unwrap();
    for command in ["variables", "setVariable", "setExpression", "exceptionInfo", "continue", "restartFrame"] {
        assert!(run(&tool, &runtime, json!({"action":"custom_request","command":command})).unwrap_err().to_string().contains("DAP_USAGE"));
    }
    run(&tool, &runtime, json!({"action":"terminate"})).unwrap();
}
...
fn thread_execution_and_assignment_options_are_not_silently_ignored() {
    for input in [
        ...
        assert!(serde_json::from_value::<DebugInput>(input).unwrap().validate().is_err());
    }
    let input: DebugInput = serde_json::from_value(json!({"action":"set_expression","expression":"text","value":""})).unwrap();
