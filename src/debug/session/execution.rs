...
impl Default for Execution {
    fn default() -> Self {
        Self {
            baseline: Cell { status: Status::Running, revision: 0 },
...
impl Execution {
    ...
    fn insert(&mut self, id: u64) -> Result<&mut Thread> {
        if !self.threads.contains_key(&id) && self.threads.len() >= MAX_THREADS {
            return Err(tool_err("DAP_THREAD_LIMIT", "debug session exceeds 4096 tracked threads"));
        }
    ...
    pub(super) fn event(&mut self, name: &str, body: &Value) -> Result<()> {
        ...
        match name {
            "stopped" => {
                ...
                if !all && id.is_none() {
                    return Err(tool_err("DAP_PROTOCOL", "single-thread stopped event omitted threadId"));
                }
                let reason = body["reason"].as_str().filter(|reason| !reason.is_empty())
                ...
                if let Some(ids) = body["hitBreakpointIds"].as_array() {
                    details["hitBreakpointIds"] = json!(ids.iter().filter_map(Value::as_i64).take(256).collect::<Vec<_>>());
                }
                let cell = Cell {
                    status: Status::Stopped(Arc::new(Stop { revision, reason, details })),
                    revision,
                };
                if all {
                    self.baseline = cell.clone();
                    for thread in self.threads.values_mut() { thread.cell = cell.clone(); }
                }
                if let Some(id) = id {
            ...
            "continued" => {
                ...
                let id = thread_id(body.get("threadId"))?;
                if !all && id.is_none() {
                    return Err(tool_err("DAP_PROTOCOL", "partial continued event omitted threadId"));
                }
                let revision = self.advance()?;
                let cell = Cell { status: Status::Running, revision };
                if all {
                    self.baseline = cell.clone();
                    for thread in self.threads.values_mut() { thread.cell = cell.clone(); }
                }
                if let Some(id) = id { self.insert(id)?.cell = cell; }
                self.choose();
            }
            "thread" => {
                let id = thread_id(body.get("threadId"))?
                ...
                match body["reason"].as_str() {
                    Some("started") => {
                        let revision = self.advance()?;
                        self.insert(id)?.cell = Cell { status: Status::Running, revision };
                    }
                    Some("exited") => {
                        self.advance()?;
    ...
    pub(super) fn observe_threads(&mut self, body: &Value, revision: u64) -> Result<()> {
        let entries = body["threads"].as_array()
            .filter(|entries| entries.len() <= MAX_THREADS)
            .ok_or_else(|| tool_err("DAP_PROTOCOL", "threads response must contain at most 4096 threads"))?;
        ...
        for entry in entries {
            ...
            if names.insert(id, name.chars().take(1024).collect::<String>()).is_some() {
                return Err(tool_err("DAP_PROTOCOL", "duplicate thread ID in inventory"));
            }
        }
        if self.exited || revision != self.revision { return Ok(()); }
        self.threads.retain(|id, _| names.contains_key(id));
        for (id, name) in names { self.insert(id)?.name = Some(name); }
        self.choose();
        Ok(())
    }
    ...
    pub(super) fn require(&self, selected: Option<u64>) -> Result<u64> {
        if self.exited {
            return Err(tool_err("DAP_STATE_EXITED", "debuggee or adapter has exited"));
        }
        if let Some(id) = selected.or(self.selected) {
            match self.threads.get(&id).map(|thread| &thread.cell.status) {
                ...
                Some(Status::Running) => return Err(tool_err("DAP_STATE_RUNNING", format!("thread {id} is running; pause it before inspection"))),
                None => return Err(tool_err("DAP_THREAD_UNKNOWN", format!("thread {id} is not in the current inventory; query threads first"))),
            }
        }
    ...
    pub(super) fn aggregate(&self) -> ExecState {
        if self.exited { return ExecState::Exited; }
        if let Some(id) = self.selected && let Some(stop) = self.stop(id) {
            return ExecState::Stopped { thread_id: id, reason: stop.reason.clone() };
        }
        if self.threads.is_empty() && let Status::Stopped(stop) = &self.baseline.status {
            return ExecState::Stopped { thread_id: 0, reason: stop.reason.clone() };
        }
        ExecState::Running
    }

    pub(super) fn snapshot(&self) -> Value {
        let threads: Vec<_> = self.threads.iter().map(|(id, thread)| {
    ...
    pub(super) fn stopped_since(&self, revision: u64, selected: Option<u64>) -> Option<(u64, String)> {
        self.threads.iter().filter(|(id, _)| selected.is_none_or(|selected| **id == selected))
            .filter_map(|(id, _)| self.stop(*id).filter(|stop| stop.revision > revision)
                .map(|stop| (*id, stop)))
    ...
    pub(super) fn resume(&mut self, thread: u64, single: bool) -> Result<Resume> {
        ...
        let revision = self.advance()?;
        let ticket = Resume {
            baseline: self.baseline.clone(), threads: self.threads.clone(),
            selected: self.selected, thread, revision,
        };
        let cell = Cell { status: Status::Running, revision };
        if single {
    ...
    pub(super) fn restore(&mut self, ticket: &Resume, except_resumed: bool) {
        ...
        if self.selected.is_none() { self.selected = ticket.selected; }
        self.choose();
    }
    ...
    pub(super) fn continued_all(&mut self, ticket: &Resume) {
        ...
        if self.baseline.revision <= ticket.revision { self.baseline = cell.clone(); }
        for thread in self.threads.values_mut() {
            if thread.cell.revision <= ticket.revision { thread.cell = cell.clone(); }
        }
        self.choose();
    }
}
...
fn boolean(body: &Value, key: &str, default: bool) -> Result<bool> {
    match body.get(key) {
        None => Ok(default),
        Some(value) => value.as_bool().ok_or_else(|| tool_err("DAP_PROTOCOL", format!("{key} must be boolean"))),
    }
}

fn thread_id(value: Option<&Value>) -> Result<Option<u64>> {
    value.map(|value| value.as_u64().filter(|id| (1..=2_147_483_647).contains(id))
        .ok_or_else(|| tool_err("DAP_PROTOCOL", "thread ID must be a positive DAP integer"))).transpose()
}
...
mod tests {
    ...
    fn stopped(execution: &mut Execution, id: u64, all: bool) {
        execution.event("stopped", &json!({"threadId":id,"reason":"breakpoint","allThreadsStopped":all})).unwrap();
    }

    fn two_threads() -> Execution {
        let mut execution = Execution::default();
        stopped(&mut execution, 7, true);
        execution.observe_threads(&json!({"threads":[{"id":7,"name":"main"},{"id":8,"name":"worker"}]}), execution.revision()).unwrap();
        execution
    }
    ...
    fn omitted_all_stopped_never_grants_other_threads_inspection() {
        let mut execution = Execution::default();
        execution.observe_threads(&json!({"threads":[{"id":7,"name":"main"},{"id":8,"name":"worker"}]}), 0).unwrap();
        execution.event("stopped", &json!({"threadId":7,"reason":"exception"})).unwrap();
    ...
    fn partial_continue_preserves_a_stopped_peer_and_default_continue_resumes_all() {
        ...
        execution.event("continued", &json!({"threadId":7,"allThreadsContinued":false})).unwrap();
        assert!(execution.require(Some(7)).is_err());
        assert_eq!(execution.require(None).unwrap(), 8);
        execution.event("continued", &json!({"threadId":8})).unwrap();
    ...
    fn late_inventory_uses_all_stop_but_never_resurrects_an_exited_thread() {
        ...
        execution.event("thread", &json!({"threadId":8,"reason":"exited"})).unwrap();
        execution.observe_threads(&json!({"threads":[{"id":8,"name":"stale"}]}), revision).unwrap();
        assert!(execution.require(Some(8)).is_err());
        execution.event("thread", &json!({"threadId":8,"reason":"started"})).unwrap();
        assert!(execution.require(Some(8)).is_err());
        assert_eq!(execution.require(Some(7)).unwrap(), 7);
    }
    ...
    fn rejected_single_resume_restores_only_its_own_transition() {
        ...
        execution.event("continued", &json!({"threadId":8,"allThreadsContinued":false})).unwrap();
    ...
    fn early_stop_wins_over_success_and_rejection_replies() {
        for rejected in [false, true] {
            ...
            if rejected { execution.restore(&ticket, false); }
            else { execution.continued_all(&ticket); }
    ...
    fn all_stop_without_thread_requires_inventory_and_invalid_events_fail() {
        let mut execution = Execution::default();
        execution.event("stopped", &json!({"reason":"pause","allThreadsStopped":true})).unwrap();
        assert_eq!(execution.require(None).unwrap(), 0);
        assert!(execution.require(Some(99)).is_err());
        assert!(execution.event("stopped", &json!({"reason":"pause"})).is_err());
        assert!(execution.event("continued", &json!({"allThreadsContinued":false})).is_err());
        assert!(execution.event("stopped", &json!({"threadId":7,"reason":"pause","allThreadsStopped":"yes"})).is_err());
    }

    #[test]
    fn termination_is_final_and_preserve_focus_keeps_the_existing_stop() {
        let mut execution = two_threads();
        execution.event("stopped", &json!({"threadId":8,"reason":"exception","preserveFocusHint":true})).unwrap();
    ...
    fn thread_limits_and_duplicate_inventory_are_not_silently_truncated() {
        let mut execution = Execution::default();
        assert!(execution.observe_threads(&json!({"threads":[{"id":7,"name":"a"},{"id":7,"name":"b"}]}), 0).is_err());
        for id in 1..=MAX_THREADS as u64 {
            execution.event("thread", &json!({"threadId":id,"reason":"started"})).unwrap();
        }
        assert!(execution.event("thread", &json!({"threadId":MAX_THREADS + 1,"reason":"started"})).is_err());
    }
}
