...
impl Handles {
    fn retain(&mut self, mut keep: impl FnMut(&Binding) -> bool) {
        ...
        self.reverse.retain(|binding, id| self.entries.get(id) == Some(binding));
    }
    ...
    fn invalidate_variables(&mut self, thread: u64) {
        self.retain(|binding| binding.kind != Kind::Variables || binding.suspension.thread != thread);
    }

    fn encode(&mut self, kind: Kind, remote: u64, suspension: Suspension) -> Result<u64> {
        if kind == Kind::Variables && remote == 0 { return Ok(0); }
        let binding = Binding { kind, remote, suspension };
        if let Some(id) = self.reverse.get(&binding) { return Ok(*id); }
        if self.entries.len() >= MAX_HANDLES {
            return Err(tool_err("DAP_HANDLE_LIMIT", "debug suspension exceeds 8192 live frame/object handles; request smaller pages"));
        }
...
fn allocate(counter: &AtomicU64) -> Result<u64> {
    counter.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |id| {
        if id <= MAX_HANDLE { Some(id + 1) } else { None }
    }).map_err(|_| tool_err("DAP_HANDLE_LIMIT", "debugger handle IDs exhausted; refusing to reuse an old ID"))
}

fn suspension(state: &State, thread: u64) -> Result<Suspension> {
    state.check()?;
    state.execution.require(Some(thread))?;
    let stop = state.execution.stamp(thread)
        .ok_or_else(|| tool_err("DAP_STATE_RUNNING", "selected thread is no longer stopped"))?;
    Ok(Suspension { thread, stop, inspection: state.inspection_revision })
}

fn remote_id(value: &Value) -> Result<u64> {
    value.as_u64().filter(|id| *id <= 2_147_483_647)
        .ok_or_else(|| tool_err("DAP_PROTOCOL", "adapter returned an invalid frame/object reference"))
}

fn map_variable(handles: &mut Handles, value: &mut Value, at: Suspension, required: bool) -> Result<()> {
    match value.get("variablesReference") {
        ...
        None if required => return Err(tool_err("DAP_PROTOCOL", "adapter omitted variablesReference")),
        None => {}
    }
...
fn rewrite_reply(handles: &mut Handles, command: &str, mut body: Value, at: Suspension) -> Result<Value> {
    if !body.is_object() { return Err(tool_err("DAP_PROTOCOL", "inspection response must be an object")); }
    match command {
        "stackTrace" => {
            let frames = body["stackFrames"].as_array_mut().filter(|frames| frames.len() <= MAX_HANDLES)
                .ok_or_else(|| tool_err("DAP_PROTOCOL", "invalid or oversized stack frame response"))?;
            for frame in frames {
                frame["id"] = json!(handles.encode(Kind::Frame, remote_id(&frame["id"])?, at)?);
            }
        }
        "scopes" | "variables" => {
            let key = if command == "scopes" { "scopes" } else { "variables" };
            let entries = body[key].as_array_mut().filter(|entries| entries.len() <= MAX_HANDLES)
                .ok_or_else(|| tool_err("DAP_PROTOCOL", "invalid or oversized scope/variable response"))?;
            for entry in entries { map_variable(handles, entry, at, true)?; }
        }
        "evaluate" | "setVariable" | "setExpression" => {
            let field = if command == "evaluate" { "result" } else { "value" };
            if !body[field].is_string() {
                return Err(tool_err("DAP_PROTOCOL", format!("adapter omitted {field} in inspection response")));
            }
            map_variable(handles, &mut body, at, command == "evaluate")?;
        }
        "exceptionInfo" => {
            if !body["exceptionId"].is_string() || !matches!(body["breakMode"].as_str(), Some("never" | "always" | "unhandled" | "userUnhandled")) {
                return Err(tool_err("DAP_PROTOCOL", "adapter returned incomplete exception information"));
            }
        }
        "dataBreakpointInfo" => {}
        _ => return Err(tool_err("DAP_USAGE", "unsupported managed inspection request")),
    }
    Ok(body)
}
...
pub(super) async fn call(session: &DapSession, command: &str, mut arguments: Value) -> Result<Value> {
    match command {
    ...
    let mut handles = OwnedMutexGuard::lock(Arc::clone(&session.inspection), owner.cx()).await
    ...
    let before = {
        ...
        let mut bound = None;
        for (field, kind) in [("frameId", Kind::Frame), ("variablesReference", Kind::Variables)] {
            if let Some(value) = arguments.get(field) {
                let local = value.as_u64()
                    .ok_or_else(|| tool_err("DAP_USAGE", "frame/object handle must be an integer"))?;
                let binding = handles.resolve(kind, local)?;
                if bound.is_some_and(|at| at != binding.suspension) {
                    return Err(tool_err("DAP_STALE_REFERENCE", "cannot combine handles from different suspended threads"));
                }
        ...
        if let Some(at) = bound {
            if selected.is_some_and(|thread| thread != at.thread) {
                return Err(tool_err("DAP_USAGE", "threadId does not own the supplied frame/object handle"));
            }
            if suspension(&state, at.thread)? != at {
                return Err(tool_err("DAP_STALE_REFERENCE", "debugger handle outlived its suspension"));
            }
            at
        } else {
            let thread = state.execution.require(selected)?;
            if thread == 0 {
                return Err(tool_err("DAP_NO_THREADS", "query threads and select a stopped thread before inspection"));
            }
    ...
    if before.stop != after.stop || (!mutating && before.inspection != after.inspection) {
        return Err(tool_err("DAP_STALE_REFERENCE", "debuggee changed suspension or invalidated inspection during the request; refresh the stack"));
    }
...
mod tests {
    ...
    fn at(stop: u64) -> Suspension {
        Suspension { thread: 7, stop, inspection: 0 }
    }
    ...
    fn recycled_adapter_ids_never_reuse_a_model_facing_handle() {
        ...
        let other_session = Handles::default().encode(Kind::Variables, 66, at(2)).unwrap();
    ...
    fn assignments_invalidate_objects_but_not_their_stack_frames() {
        ...
        let object = handles.encode(Kind::Variables, 66, at(1)).unwrap();
        let peer = handles.encode(Kind::Variables, 77, Suspension { thread: 8, ..at(1) }).unwrap();
    ...
    fn reply_rewriting_preserves_values_and_rejects_missing_references() {
        ...
        assert!(rewrite_reply(&mut handles, "variables", json!({"variables":[{"name":"x","value":"1"}]}), at(1)).is_err());
        assert!(rewrite_reply(&mut handles, "exceptionInfo", json!({"exceptionId":"Error"}), at(1)).is_err());
    }
