...
const RELEASE_SCRIPT: &str = "IFS= read -r outcome\nif [ \"$outcome\" = complete ]; then exit 0; fi\nexec \"$@\"\n";
...
impl ReleaseGuard {
    fn arm(
        ...
        let executable = helpers.get("xdotool").map_or(Path::new("xdotool"), PathBuf::as_path);
        let mut command = Command::new("/bin/sh");
        command.args(["-c", RELEASE_SCRIPT, "pi-desktop-release"])
            .arg(executable).args(release).current_dir(cwd).process_group(0)
            .stdin(Stdio::piped()).stdout(Stdio::null()).stderr(Stdio::null());
        let child = owner.process().spawn_checked(&mut command)?;
        let mut guard = Self { child: Some(child), stdin: None };
    ...
    fn complete(&mut self) -> Result<()> {
        self.stdin.as_mut().ok_or_else(|| error("desktop release guard closed early"))?
...
async fn dispatch(
    ...
    check_owner(owner)?;
    let guard = if release.is_empty() { None } else {
        Some(ReleaseGuard::arm(owner, cwd, helpers, release)?)
    };
    let mut transaction = InputTransaction { running: None, release: guard };
    transaction.running = Some(process::start(owner, cwd, helpers, "xdotool", command, input)?);
    let deadline = Instant::now() + Duration::from_secs(30);
    loop {
        check_owner(owner)?;
        if Instant::now() >= deadline {
            return Err(error("desktop input helper timed out; side effects may already have occurred"));
        }
        if let Some(status) = transaction.running.as_mut().expect("owned input helper").poll(4096)? {
            if !status.success() {
                return Err(error(format!("desktop input helper failed ({status}); releases were attempted")));
            }
...
fn input_command(args: &Value) -> Result<InputCommand> {
    ...
    let mut command = InputCommand { args: Vec::new(), stdin: Vec::new(), release: Vec::new() };
    ...
    match action {
        ...
        "mouse_drag" => {
            ...
            command.args = strings(&["mousedown", button, "mousemove", "--sync", &x, &y, "mouseup", button]);
            command.release = strings(&["mouseup", button]);
        }
        "key_press" => {
        ...
        "key_type" => {
            ...
            for ch in value.chars() {
                keys.insert(match ch {
                    '\n' => "Return".to_string(), '\t' => "Tab".to_string(),
                    ' ' => "space".to_string(),
                    ch if ch.is_ascii_alphanumeric() => ch.to_string(),
                    ch => format!("U{:04X}", u32::from(ch)),
                });
            }
            command.release = strings(&["keyup", "Shift_L", "Shift_R", "Control_L", "Control_R", "Alt_L", "Alt_R", "Super_L", "Super_R"]);
            command.release.extend(keys);
        }
        "scroll" => {
            let wheel = match args.get("direction").and_then(Value::as_str).unwrap_or("down") {
                "up" => "4", "down" => "5", "left" => "6", "right" => "7",
                _ => return Err(error("unsupported scroll direction")),
            };
            let amount = args.get("amount").and_then(Value::as_u64).unwrap_or(3).to_string();
            command.args = strings(&["click", "--repeat", &amount, "--delay", "10", wheel]);
            command.release = strings(&["mouseup", wheel]);
        }
        "focus_window" => {
            let window = args["window_id"].as_u64().expect("validated window ID").to_string();
...
pub(super) async fn execute(
    ...
    let requested_window = args.get("window_id").and_then(Value::as_u64)
    ...
    {
        return Err(error("target window is not focused; focus_window explicitly before sending input"));
    }
    dispatch(owner, cwd, helpers, &command.args, &command.stdin, &command.release).await?;
    if action == "focus_window" && active_window(owner, cwd, helpers).await? != requested_window {
        return Err(error("window activation did not produce the requested focus"));
    }
    let position = if matches!(action, "mouse_move" | "mouse_drag") {
        let raw = query(owner, cwd, helpers, "xdotool", &["getmouselocation", "--shell"]).await?;
        let position = parse_position(&raw)?;
        if args["x"].as_i64() != Some(position.0) || args["y"].as_i64() != Some(position.1) {
            return Err(error("pointer did not reach the requested position; it may be constrained by the desktop"));
        }
        Some(json!({"x":position.0,"y":position.1}))
    } else { None };
    Ok(output(format!("Dispatched desktop {action} through X11; application-level effects are not inferred"), json!({
...
fn parse_position(raw: &str) -> Result<(i64, i64)> {
    ...
    for line in raw.lines() {
        if let Some((name, value)) = line.split_once('=') {
            let target = match name { "X" => &mut x, "Y" => &mut y, _ => continue };
            if target.is_some() { return Err(error("duplicate pointer coordinate")); }
            *target = Some(value.parse::<i64>().map_err(|_| error("invalid pointer coordinate"))?);
        }
    }
...
async fn clipboard_write(
    ...
    let mut candidate = process::start(owner, cwd, helpers, "xclip",
        &strings(&["-selection", "clipboard", "-in", "-quiet", "-target", "UTF8_STRING"]), value.as_bytes())?;
    for _ in 0..50 {
        check_owner(owner)?;
        if candidate.poll(4096)?.is_some() {
            return Err(error("clipboard selection owner exited before the write was verified"));
        }
        let bytes = process::run(owner, cwd, helpers, "xclip",
            &strings(&["-selection", "clipboard", "-out", "-target", "UTF8_STRING"]), b"", 64 * 1024).await?;
        if bytes == value.as_bytes() && candidate.poll(4096)?.is_none() {
            ...
            return Ok(output(format!("Copied {} characters to the X11 clipboard and verified the contents", value.chars().count()), json!({
    ...
    Err(error("clipboard contents did not match the requested text; the desktop may have another clipboard owner"))
}
...
mod tests {
    ...
    fn typed_text_is_stdin_not_a_command_or_argument() {
        ...
        assert_eq!(command.args, strings(&["type","--delay","1","--clearmodifiers","--file","-"]));
        assert!(!command.release.iter().any(|arg| arg == "exec" || arg.contains("touch")));
    }

    #[test]
    fn drag_and_scroll_pair_press_with_release() {
        let drag = input_command(&json!({"action":"mouse_drag","x":42,"y":31,"button":"right"})).unwrap();
        assert_eq!(drag.args, strings(&["mousedown","3","mousemove","--sync","42","31","mouseup","3"]));
        assert_eq!(drag.release, strings(&["mouseup","3"]));
        assert!(input_command(&json!({"action":"mouse_move","x":-1,"y":5})).is_err());
        let scroll = input_command(&json!({"action":"scroll","direction":"up","amount":5})).unwrap();
        assert_eq!(scroll.args, strings(&["click","--repeat","5","--delay","10","4"]));
    }

    #[test]
    fn position_data_is_parsed_and_never_evaluated() {
        assert_eq!(parse_position("X=42\nY=31\nSCREEN=0\nWINDOW=17\n").unwrap(), (42,31));
    ...
    fn dropping_input_transaction_stops_input_then_runs_the_armed_release() {
        ...
        let guard = ReleaseGuard::arm(&owner, dir.path(), &helpers, &strings(&["mouseup","1"])).unwrap();
        let running = process::start(&owner, dir.path(), &helpers, "xdotool", &strings(&["hold"]), b"").unwrap();
        let deadline = Instant::now() + Duration::from_secs(5);
        while !dir.path().join("started").exists() && Instant::now() < deadline {
            std::thread::sleep(Duration::from_millis(2));
        }
        assert!(dir.path().join("started").exists());
        let mut transaction = InputTransaction { running: Some(running), release: Some(guard) };
        owner.cancel_with(asupersync::types::CancelKind::User, Some("cancel input"));
        assert!(transaction.running.as_mut().unwrap().child.try_wait().is_err());
        drop(transaction);
        assert_eq!(std::fs::read(dir.path().join("released")).unwrap(), b"released");
    }
    ...
    fn successful_input_disarms_cleanup() {
        ...
        std::fs::set_permissions(&helper, std::fs::Permissions::from_mode(0o700)).unwrap();
        let helpers = BTreeMap::from([("xdotool".to_string(), helper)]);
        let mut guard = ReleaseGuard::arm(&AgentCx::for_request(), dir.path(), &helpers, &strings(&["mouseup","1"])).unwrap();
