...
fn storage_error() -> Error {
    failure("PI_SHARED_MEMORY_STORAGE", "Shared memory storage operation failed")
}
...
impl SharedMemoryStore {
        ...
        self.with_conn(|conn| super::transactions::run(conn, |conn| {
            ...
            if old.is_some() {
                conn.execute_sync(
                    ...
                    &[text(content), SqliteValue::Integer(bytes), text(&revision),
                      SqliteValue::Integer(updated_at_ms), text(&self.session_id), text(key)],
                ).map_err(|_| storage_error())?;
            } else {
                conn.execute_sync(
    ...
    pub fn list(&self, prefix: &str, after: Option<&str>, limit: usize) -> Result<SharedMemoryPage> {
        if !prefix.is_empty() { validate_key(prefix)?; }
        if let Some(after) = after { validate_key(after)?; }
        if !(1..=MAX_LIST_LIMIT).contains(&limit) {
            return Err(failure("PI_SHARED_MEMORY_LIST_LIMIT", "List limit must be between 1 and 50"));
        }
...
fn version_from_row(row: &Row) -> Result<SharedMemoryVersion> {
    ...
    if bytes > MAX_VALUE_BYTES { return Err(storage_error()); }
...
fn check_quota(
    ...
    let (sql, params, max_keys, max_bytes) = match session {
        Some(session) => (
...
pub(super) fn session_requested(input: &Value) -> Result<bool> {
    match input.get("scope") {
        ...
        _ => Err(failure("PI_SHARED_MEMORY_INVALID_SCOPE", "scope must be 'project' or 'session'")),
    }
}
...
async fn bound_store(bank: &Arc<MemoryStore>, scope: Option<&JobSessionScope>) -> Result<SharedMemoryStore> {
    ...
    let checkpoint = || owner.checkpoint().map_err(|_| {
        failure("PI_SHARED_MEMORY_CANCELLED", "Shared memory operation cancelled before dispatch")
    });
    checkpoint()?;
    if !owner.capabilities().io {
        return Err(failure("PI_SHARED_MEMORY_PERMISSION", "Shared memory requires I/O capability"));
    }
    let scope = scope.ok_or_else(|| failure(
        "PI_SHARED_MEMORY_SESSION_UNAVAILABLE", "Shared memory requires a bound session; no global fallback is used",
    ))?;
    let session_id = scope.session_id().await.map_err(|_| failure(
        "PI_SHARED_MEMORY_SESSION_UNAVAILABLE", "The current session identity is unavailable",
    ))?;
    checkpoint()?;
...
pub(super) async fn write_output(
    bank: &Arc<MemoryStore>, scope: Option<&JobSessionScope>, input: Value,
) -> Result<ToolOutput> {
    let input: WriteInput = parse_input(input)?;
    let store = bound_store(bank, scope).await?;
    let version = store.write(&input.key, &input.content, input.expected_revision.as_deref())?;
    Ok(super::text_output(
        format!("Stored shared key {} ({} bytes); revision {}", version.key, version.bytes, version.revision),
        json!({"schema": SHARED_MEMORY_SCHEMA, "scope": "session", "value": version}), false,
    ))
}
...
pub(super) async fn read_or_list_output(
    ...
    if let Some(key) = input.key {
        if input.prefix.is_some() || input.after.is_some() || input.limit.is_some() {
            return Err(failure("PI_SHARED_MEMORY_INVALID_INPUT", "A key read cannot also request listing options"));
        }
        let value = store.read(&key)?.ok_or_else(|| failure(
            "PI_SHARED_MEMORY_NOT_FOUND", "No shared value exists for this key in the current session",
        ))?;
        return Ok(super::text_output(
            value.content.clone(),
            json!({"schema": SHARED_MEMORY_SCHEMA, "scope": "session", "value": value}), false,
        ));
    }
    let page = store.list(input.prefix.as_deref().unwrap_or(""), input.after.as_deref(), input.limit.unwrap_or(25))?;
    let lines = page.entries.iter().map(|entry| {
        format!("{} ({} bytes), revision {}", entry.version.key, entry.version.bytes, entry.version.revision)
    }).collect::<Vec<_>>().join("\n");
    Ok(super::text_output(
        if lines.is_empty() { "No shared keys in this session match the request.".to_string() } else { lines },
        json!({"schema": SHARED_MEMORY_SCHEMA, "scope": "session", "entries": page.entries, "nextCursor": page.next_cursor}), false,
    ))
}

#[derive(Clone, Copy)]
enum Operation { Read, Write, List }

/// Dedicated native tools for SDK/custom registries. Registration binds the
/// same live session scope as the existing registry tools; standalone callers
/// must explicitly call `bind_job_session_scope`. The built-in retain/recall
/// surfaces offer the same implementation with `scope: "session"`.
...
impl SharedMemoryTool {
    ...
    pub const fn read(bank: Arc<MemoryStore>) -> Self {
        Self { bank, scope: None, operation: Operation::Read }
    }
    #[must_use]
    pub const fn write(bank: Arc<MemoryStore>) -> Self {
        Self { bank, scope: None, operation: Operation::Write }
    }
    #[must_use]
    pub const fn list(bank: Arc<MemoryStore>) -> Self {
        Self { bank, scope: None, operation: Operation::List }
    }
}
...
impl Tool for SharedMemoryTool {
    fn name(&self) -> &str {
        match self.operation { Operation::Read => "read_memory", Operation::Write => "write_memory", Operation::List => "list_memory" }
    }
    fn label(&self) -> &str { self.name() }
    fn description(&self) -> &str {
        match self.operation {
            Operation::Read => "Read a shared key and its revision from the current session; other sessions are inaccessible.",
            Operation::Write => "Write exact text to a shared session key. Use expectedRevision from a prior read, or 'absent' to create only. Values survive compaction but never become project facts automatically.",
            Operation::List => "List shared session keys with bounded previews, revisions, literal prefix filtering and key cursors.",
        }
    }
    fn parameters(&self) -> Value {
        match self.operation {
            Operation::Read => json!({"type":"object","properties":{"key":{"type":"string","maxLength":128}},"required":["key"],"additionalProperties":false}),
    ...
    fn effects(&self) -> ToolEffects {
        match self.operation { Operation::Write => ToolEffects::write(), _ => ToolEffects::read() }
    }
    fn bind_job_session_scope(&mut self, scope: JobSessionScope) { self.scope = Some(scope); }
