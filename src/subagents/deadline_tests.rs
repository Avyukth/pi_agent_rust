...
fn fixture(script: &str, timeout: Duration) -> (TempDir, SubagentTool) {
    ...
    let tool = SubagentTool::with_paths(root.path().to_path_buf(), global, binary).with_timeout(timeout);
    (root, tool)
}
...
fn request() -> Value { json!({"agent":"worker","task":"finish"}) }

fn run(tool: &SubagentTool, input: Value, callback: Option<Box<dyn Fn(ToolUpdate) + Send + Sync>>) -> Result<ToolOutput> {
...
fn results(output: &ToolOutput) -> &Vec<Value> {
    output.details.as_ref().unwrap()["results"].as_array().unwrap()
}

fn assert_timeout(value: &Value) {
    assert_eq!(value["status"], "failed", "{value}");
    assert!(value["error"].as_str().unwrap().contains("PI_SUBAGENT_TIMEOUT"), "{value}");
}

#[test]
fn invalid_request_timeouts_never_launch() {
    let (_root, tool) = fixture("touch launched", Duration::from_secs(5));
    assert_eq!(tool.parameters()["properties"]["timeoutSeconds"]["maximum"], 86_400);
    for timeout in [json!(0), Value::Null, json!(-1), json!(1.5), json!("5"), json!(86_401)] {
...
fn an_uncooperative_running_child_is_terminated_by_the_parent() {
    let (_root, tool) = fixture("printf 'started\\n' > launched\nexec sleep 30", Duration::from_secs(1));
    ...
    assert!(tool.cwd.join("launched").exists());
    assert!(started.elapsed() < Duration::from_secs(10), "must not wait for sleep 30");
...
fn a_terminal_frame_cannot_hide_a_process_that_never_exits() {
    let (_root, tool) = fixture(&format!("{}exec sleep 30", end("looks complete")), Duration::from_secs(1));
    let output = run(&tool, request(), None).unwrap();
    assert!(output.is_error);
...
fn queued_parallel_work_consumes_the_original_budget() {
    let (_root, tool) = fixture("printf 'started\\n' >> launches\nexec sleep 30", Duration::from_secs(1));
    let output = run(&tool, json!({"concurrency":1,"tasks":[
    ...
    assert!(results(&output)[1]["pid"].is_null(), "expired queued work must never spawn");
    assert_eq!(std::fs::read_to_string(tool.cwd.join("launches")).unwrap(), "started\n");
}
...
fn starting_callback_can_exhaust_budget_before_spawn() {
    ...
    let output = run(&tool, request(), Some(Box::new(move |update| {
...
fn chained_steps_do_not_get_a_fresh_timeout() {
    let (_root, tool) = fixture(&format!("printf 'started\\n' >> launches\n{}", end("first")), Duration::from_secs(1));
    let observed = Arc::new(AtomicBool::new(false));
    let flag = Arc::clone(&observed);
    let output = run(&tool, json!({"chain":[
        {"agent":"worker","task":"first"}, {"agent":"worker","task":"second"}
    ]}), Some(Box::new(move |update| {
        if update.details.as_ref().is_some_and(|value| value["result"]["status"] == "completed")
...
fn an_expired_budget_does_not_launch_a_corrective_retry() {
    let (_root, tool) = fixture(&format!("printf 'started\\n' >> launches\n{}", end("not JSON")), Duration::from_secs(1));
    let observed = Arc::new(AtomicBool::new(false));
    let flag = Arc::clone(&observed);
    let mut input = request();
    input["outputSchema"] = json!({"type":"object"});
    let output = run(&tool, input, Some(Box::new(move |update| {
        if update.details.as_ref().is_some_and(|value| value["result"]["schemaValid"] == false)
            && !flag.swap(true, Ordering::SeqCst)
        {
            std::thread::sleep(Duration::from_millis(1100));
        }
...
fn expired_isolated_work_is_preserved_and_never_applied() {
    ...
    for args in [
        ...
        assert!(Command::new("git").args(args).current_dir(&tool.cwd).status().unwrap().success());
    }
    ...
    assert_eq!(std::fs::read_to_string(retained.join("tracked.txt")).unwrap(), "unaccepted\n");
}

#[test]
fn background_tan_uses_the_same_host_budget() {
    let (_root, tool) = fixture("exec sleep 30", Duration::from_secs(1));
    let completion = asupersync::runtime::RuntimeBuilder::current_thread().build().unwrap()
        .block_on(tool.run_background_tan("bounded background work")).unwrap();
