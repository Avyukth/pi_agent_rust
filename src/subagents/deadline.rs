...
impl Deadline {
    ...
    pub(super) fn for_request(
        ...
        Self::at(host, requested_seconds, inherited, Instant::now(), SystemTime::now())
    }
    ...
    fn at(
        ...
        let limit = requested_seconds.map(seconds).transpose()?.map_or(host, |request| host.min(request));
        let wall_ms = u64::try_from(wall.duration_since(UNIX_EPOCH)
            .map_err(|_| inherited_error())?.as_millis()).map_err(|_| inherited_error())?;
        ...
        if let Some(inherited) = inherited_ms {
            if inherited == 0 { return Err(inherited_error()); }
    ...
    fn check_at(self, now: Instant) -> std::result::Result<(), &'static str> {
        if now >= self.at { Err(TIMED_OUT) } else { Ok(()) }
    }
    ...
    pub(super) fn configure_child(self, command: &mut Command) -> std::result::Result<(), &'static str> {
        self.check()?;
        let remaining = self.remaining();
        if remaining.is_zero() { return Err(TIMED_OUT); }
        let whole_seconds = remaining.as_secs() + u64::from(remaining.subsec_nanos() != 0);
        command.arg("--max-time").arg(whole_seconds.to_string())
...
mod tests {
    ...
    fn wall() -> SystemTime { UNIX_EPOCH + Duration::from_secs(1_000) }

    #[test]
    fn request_can_shorten_but_never_extend_host_policy() {
        let now = Instant::now();
        for (request, expected) in [(None, 10), (Some(2), 2), (Some(60), 10)] {
            let deadline = Deadline::at(Duration::from_secs(10), request, None, now, wall()).unwrap();
            assert_eq!(deadline.at.duration_since(now), Duration::from_secs(expected));
        }
    }
    ...
    fn inherited_ceiling_caps_every_new_local_budget() {
        let now = Instant::now();
        let deadline = Deadline::at(DEFAULT_TIMEOUT, Some(300), Some(1_003_000), now, wall()).unwrap();
        assert_eq!(deadline.at.duration_since(now), Duration::from_secs(3));
        assert_eq!(deadline.expires_unix_ms, 1_003_000);
        let later = Deadline::at(DEFAULT_TIMEOUT, None, Some(deadline.expires_unix_ms),
            now + Duration::from_secs(2), wall() + Duration::from_secs(2)).unwrap();
        assert_eq!(later.at, deadline.at, "nested work must not restart the budget");
    }
    ...
    fn copying_for_queue_and_retry_does_not_reset_expiry() {
        ...
        assert_eq!(queued.check_at(now + Duration::from_secs(5)), Err(TIMED_OUT));
        assert_eq!(retry.at, original.at);
    }

    #[test]
    fn invalid_limits_are_rejected_without_echoing_input() {
        for raw in ["", "0", "-1", "1.5", " 30", "30 ", "86401", "999999999999999999999", "credential-secret"] {
        ...
        for raw in ["1", "900", "86400"] { assert!(parse_seconds(raw).is_ok()); }
        for limit in [Duration::ZERO, Duration::from_nanos(1), Duration::from_secs(86_401)] {
    ...
    fn child_configuration_replaces_ambient_deadline_and_never_uses_zero_seconds() {
        let deadline = Deadline::at(Duration::from_secs(2), None, None, Instant::now(), SystemTime::now()).unwrap();
        ...
        let inherited = command.get_envs().find(|(key, _)| *key == DEADLINE_ENV).unwrap().1.unwrap();
        assert_eq!(inherited.to_str().unwrap().parse::<u64>().unwrap(), deadline.expires_unix_ms);
    }
    ...
    fn invalid_epoch_and_unrepresentable_clock_fail_closed() {
        ...
        assert!(Deadline::at(DEFAULT_TIMEOUT, None, None, now, UNIX_EPOCH - Duration::from_secs(1)).is_err());
    }
}
