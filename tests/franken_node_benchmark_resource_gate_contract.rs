...
fn benchmark_resource_gate_workload_protocol_is_complete() {
    let contract = load_contract();
    validate_workload_protocol(&contract).unwrap_or_else(|err| assert!(false, "{err}"));
}

#[test]
fn benchmark_resource_gate_metric_and_confidence_contract_is_complete() {
    let contract = load_contract();
    validate_metric_and_confidence_contract(&contract).unwrap_or_else(|err| assert!(false, "{err}"));
}

#[test]
fn benchmark_resource_gate_gate_and_evidence_contract_is_fail_closed() {
    let contract = load_contract();
    validate_gate_and_evidence_contract(&contract).unwrap_or_else(|err| assert!(false, "{err}"));
}
