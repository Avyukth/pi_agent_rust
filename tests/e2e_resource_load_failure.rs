...
fn e2e_resource_load_failure_modes() {
    ...
    report_lines.push("=== E2E RESOURCE LOAD FAILURE COMPARISON (gh #223 / bd-fpaso) ===".to_string());
    report_lines.push(format!("Settings (broken): {broken_settings_content}"));
    report_lines.push(format!("Prefix (non-writable): {}", non_writable_prefix.display()));
    ...
    let pass_c1_warning = case1_out.stderr.contains("Warning: Failed to load skills/prompts/themes/extensions:");
    ...
    report_lines.push(format!("[{}] case 1 exit code == 4 (actual: {})", if pass_c1_exit { "PASS" } else { "FAIL" }, case1_out.exit_code));
    report_lines.push(format!("[{}] case 1 stderr contains warning", if pass_c1_warning { "PASS" } else { "FAIL" }));
    report_lines.push(format!("[{}] case 1 exactly one fatal record on stdout (actual: {})", if pass_c1_rec_count { "PASS" } else { "FAIL" }, case1_records.len()));
    report_lines.push(format!("[{}] case 1 record code == 'resource.load_failed'", if pass_c1_rec_code { "PASS" } else { "FAIL" }));
    report_lines.push(format!("[{}] case 1 record exit_code == 4", if pass_c1_rec_exit { "PASS" } else { "FAIL" }));
    report_lines.push(format!("[{}] case 1 record phase == 'startup'", if pass_c1_rec_phase { "PASS" } else { "FAIL" }));
    report_lines.push(String::new());
    ...
    let pass_c2_warning = case2_out.stderr.contains("Warning: Failed to load skills/prompts/themes/extensions:");
    ...
    report_lines.push(format!("[{}] case 2 exit code == 1 (actual: {})", if pass_c2_exit { "PASS" } else { "FAIL" }, case2_out.exit_code));
    report_lines.push(format!("[{}] case 2 stderr contains warning", if pass_c2_warning { "PASS" } else { "FAIL" }));
    report_lines.push(format!("[{}] case 2 exactly one fatal record (no double-report, actual: {})", if pass_c2_rec_count { "PASS" } else { "FAIL" }, case2_records.len()));
    report_lines.push(format!("[{}] case 2 record exit_code == 1", if pass_c2_rec_exit { "PASS" } else { "FAIL" }));
    report_lines.push(String::new());
    ...
    let pass_c3_warning = !case3_out.stderr.contains("Warning: Failed to load skills/prompts/themes/extensions:");
    let pass_c3_no_rec = case3_records.is_empty();

    report_lines.push(format!("[{}] case 3 exit code == 0 (actual: {})", if pass_c3_exit { "PASS" } else { "FAIL" }, case3_out.exit_code));
    report_lines.push(format!("[{}] case 3 stderr has no warning", if pass_c3_warning { "PASS" } else { "FAIL" }));
    report_lines.push(format!("[{}] case 3 stdout has no error records (actual: {})", if pass_c3_no_rec { "PASS" } else { "FAIL" }, case3_records.len()));
    ...
    assert!(pass_c1_exit, "Case 1 exit code must be 4, got {}", case1_out.exit_code);
    assert!(pass_c1_warning, "Case 1 stderr must contain warning, got:\n{}", case1_out.stderr);
    assert!(pass_c1_rec_count, "Case 1 must have 1 fatal record, got {}", case1_records.len());
    assert!(pass_c1_rec_code, "Case 1 record code must be resource.load_failed, got {:?}", case1_records.first());
    assert!(pass_c1_rec_exit, "Case 1 record exit_code must be 4, got {:?}", case1_records.first());
    assert!(pass_c1_rec_phase, "Case 1 record phase must be startup, got {:?}", case1_records.first());

    assert!(pass_c2_exit, "Case 2 exit code must be 1, got {}", case2_out.exit_code);
    assert!(pass_c2_warning, "Case 2 stderr must contain warning, got:\n{}", case2_out.stderr);
    assert!(pass_c2_rec_count, "Case 2 must have 1 fatal record, got {}", case2_records.len());
    assert!(pass_c2_rec_exit, "Case 2 record exit_code must be 1, got {:?}", case2_records.first());

    assert!(pass_c3_exit, "Case 3 exit code must be 0, got {}", case3_out.exit_code);
    assert!(pass_c3_warning, "Case 3 stderr must not contain warning, got:\n{}", case3_out.stderr);
    assert!(pass_c3_no_rec, "Case 3 stdout must have no error records, got {}", case3_records.len());
}
