    ...
    loop {
        match listener.accept() {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {
                assert!(Instant::now() < deadline, "CDP fixture received no connection");
...
fn check_parameters(request: &Value, response: &mut Value) {
    ...
    if let Some(expected) = fields.remove("_upload_contents") {
        ...
        for (path, content) in paths.iter().zip(contents) {
            assert_eq!(std::fs::read(path.as_str().unwrap()).unwrap(), content.as_str().unwrap().as_bytes());
        }
        if let Some(destination) = fields.remove("_capture_paths_to") {
            std::fs::write(destination.as_str().unwrap(), serde_json::to_vec(paths).unwrap()).unwrap();
        }
    }
}
...
fn peer(script: Script) -> (String, thread::JoinHandle<()>) {
    ...
    let handle = thread::spawn(move || {
        ...
        for (method, mut response) in script {
            ...
            if method.starts_with("Runtime.") || method.starts_with("Page.") || method.starts_with("DOM.") {
            ...
            if method == "Runtime.evaluate" {
                ...
                assert!(matches!(expression, "({answer: 6 * 7})" | "({url: location.href, title: document.title})"));
            }
...
fn native_page_exports_send_options_and_preserve_returned_bytes() {
    ...
    for is_pdf in [false, true] {
        let mut script = attached_script();
        let args = if is_pdf {
            script.push(("Page.printToPDF", json!({
            ...
            script.push(("Page.getLayoutMetrics", json!({"result":{
        ...
        let tool = BrowserTool::new(dir.path()).with_mock(false).with_cdp_endpoint(endpoint);
        let runtime = asupersync::runtime::RuntimeBuilder::current_thread().build().unwrap();
        let result = runtime.block_on(tool.execute("export", args, None)).unwrap();
        let expected = if is_pdf { pdf.to_vec() } else { base64::engine::general_purpose::STANDARD.decode(PNG).unwrap() };
        let name = if is_pdf { "report.pdf" } else { "full.png" };
        assert_eq!(std::fs::read(dir.path().join(name)).unwrap(), expected);
        assert_eq!(result.details.as_ref().unwrap()["preview_included"], !is_pdf);
        handle.join().unwrap();
    }
}
...
fn native_export_failure_never_clobbers_or_publishes_partial_output() {
    for existing in [false, true] {
        let mut script = attached_script();
        if !existing {
            script.push(("Page.captureScreenshot", json!({"result":{"data":"bm90IGEgcG5n"}})));
        }
        let (endpoint, handle) = peer(script);
        let dir = tempfile::tempdir().unwrap();
        ...
        let tool = BrowserTool::new(dir.path()).with_mock(false).with_cdp_endpoint(endpoint);
        let runtime = asupersync::runtime::RuntimeBuilder::current_thread().build().unwrap();
        assert!(runtime.block_on(tool.execute("bad-export", json!({
            "action":"screenshot","tab":"page-1","output_path":"capture.png"
        }), None)).is_err());
        if existing {
...
fn frame_reply(url: &str) -> (&'static str, Value) {
    ("Page.getFrameTree", json!({"result":{"frameTree":{"frame":{
...
fn file_control_script(action: &str, files: Value) -> Script {
    vec![
        ...
        ("Page.createIsolatedWorld", json!({"result":{"executionContextId":4}})),
        ("DOM.resolveNode", json!({"_params":{"backendNodeId":7,"executionContextId":4},"result":{"object":{"objectId":"file-object"}}})),
...
fn upload_prefix() -> Script {
    ...
    script.extend([
        ...
        ("DOM.querySelector", json!({"_params":{"nodeId":1,"selector":"#upload"},"result":{"nodeId":2}})),
        ("DOM.describeNode", json!({"result":{"node":{"backendNodeId":7}}})),
    ]);
    script.extend(file_control_script("file_input", json!([{ "name":"previous.txt", "size":3 }])));
    script.push(frame_reply("https://example.com/"));
    script
}
...
fn native_upload_selects_private_bytes_and_retains_them_until_tool_drop() {
    ...
    script.push(("DOM.setFileInputFiles", json!({
        "_params":{"backendNodeId":7},"_upload_contents":["real upload"],
        "_capture_paths_to":capture.display().to_string(),"result":{}
    })));
    script.extend(file_control_script("file_input", json!([{ "name":"upload.txt", "size":11 }])));
    script.push(frame_reply("https://example.com/"));
    let (endpoint, handle) = peer(script);
    let tool = BrowserTool::new(dir.path()).with_mock(false).with_cdp_endpoint(endpoint);
    let runtime = asupersync::runtime::RuntimeBuilder::current_thread().build().unwrap();
    let result = runtime.block_on(tool.execute("upload", json!({
        "action":"upload","tab":"page-1","selector":"#upload","files":["upload.txt"]
    }), None)).unwrap();
    assert_eq!(result.details.as_ref().unwrap()["file_count"], 1);
    handle.join().unwrap();
    let paths: Vec<std::path::PathBuf> = serde_json::from_slice(&std::fs::read(capture).unwrap()).unwrap();
    ...
    drop(tool);
    assert!(!paths[0].exists(), "the session must own upload staging cleanup");
}
...
fn empty_upload_uses_native_clear_instead_of_noop_empty_cdp_file_list() {
    ...
    let tool = BrowserTool::new(dir.path()).with_mock(false).with_cdp_endpoint(endpoint);
    let runtime = asupersync::runtime::RuntimeBuilder::current_thread().build().unwrap();
    let result = runtime.block_on(tool.execute("clear-upload", json!({
...
fn upload_rechecks_current_origin_before_reading_any_local_file() {
    ...
    let runtime = asupersync::runtime::RuntimeBuilder::current_thread().build().unwrap();
    let error = runtime.block_on(tool.execute("blocked-upload", json!({
